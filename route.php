...
class Route {
    ...
    public static function parse_url(){
        ...
        $reguest_uri = str_replace([$dirname,$baseline],null,$_SERVER['REQUEST_URI']);
        return $reguest_uri;	
    }
    ...
    public static function run($url, $callback, $method = 'get'){
        ...
        if(in_array($_SERVER['REQUEST_METHOD'],$method)){
            ...
            //if (preg_match('@^' . $url . '$@',$reguest_uri,$parameters,PREG_OFFSET_CAPTURE)){
                if( preg_match('@^' . $url . '$@',$reguest_uri,$parameters)){
                echo "Eşleşti."; // buraya girmedi bir yerde hata var. (.htaccess ten kaynaklı olabilir)
                unset($parameters[0]);
